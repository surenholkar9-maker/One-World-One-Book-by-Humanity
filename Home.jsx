...
export default function Home() {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!project) {
      alert("Still loading project data. Please wait a moment and try again.");
      return;
    }
    setLoading(true);
    setSubmitError(null);
    try {
      const score =
        formData.content_type === "Chapter" ? 100 :
        formData.content_type === "Paragraph" ? 30 :
        formData.content_type === "Art" ? 50 :
        formData.content_type === "Idea" ? 20 : 10;
      ...
      await Project.update(project.id, {
        total_contributions: (project.total_contributions || 0) + 1,
      });
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.9)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 24 }}>
          <div style={{ background: "#0f0f1a", border: "1px solid #3a3a2a", padding: 40, maxWidth: 560, width: "100%", maxHeight: "90vh", overflowY: "auto" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
              ...
              <button onClick={() => { setShowForm(false); setSubmitError(null); }} style={{ background: "none", border: "none", color: "#666", fontSize: 24, cursor: "pointer" }}>×</button>
            </div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {/* ERROR MESSAGE */}
              {submitError && (
                <div style={{ background: "#2a0a0a", border: "1px solid #5a1a1a", color: "#e08080", padding: "12px 16px", marginBottom: 16, fontSize: 14, lineHeight: 1.5 }}>
                  ⚠️ {submitError}
                </div>
              )}
              ...
              <button
                ...
                style={{
                  width: "100%",
                  background: (loading || dataLoading) ? "#555" : "linear-gradient(135deg, #c0a060, #e8c870)",
                  color: "#0a0a0f",
                  border: "none",
                  padding: "16px",
                  fontSize: "1rem",
              ...
              >
                {loading ? "Submitting..." : dataLoading ? "Loading..." : "✍️ SUBMIT TO HISTORY"}
              </button>
